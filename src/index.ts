 ...
 *
 * Registers 11 core CAD tools with the MCP SDK.
 * Run: node dist/index.js  (or via npx cad-mcp)
 */
...
const server = new McpServer({ name: "cad-mcp", version: "1.0.0" });
...
server.registerTool(
  "cad_export_model",
  {
    description: "Export a model to STL, STEP, OBJ, GLTF, DXF, or SVG.",
    inputSchema: {
      name:        ModelName,
  ...
  "cad_query_properties",
  {
    ...
    inputSchema: {
      ...
      density: z.number().positive().optional()
        .describe("g/cm³ — e.g. steel=7.85, aluminium=2.7, PLA=1.24"),
    },
  },
  ...
  "cad_apply_operation",
  {
    ...
    inputSchema: {
      ...
      op_params:   z.record(z.unknown()).optional().default({})
        .describe(
          "Op params — fillet:{radius}, chamfer:{length}, shell:{thickness}, " +
          "extrude:{distance}, revolve:{angle}, boolean_*:{other_model}, " +
          "mirror:{plane}, pattern_linear:{direction,spacing,count}, pattern_circular:{radius,count}"
        ),
    },
  },
...
server.registerTool(
  "cad_list_templates",
  {
    description: "List available model templates (mechanical, architectural, organic).",
    inputSchema: {
...
server.registerTool(
  "cad_load_template",
  {
    description: "Instantiate a parametric template with custom parameter values.",
    inputSchema: {
...
server.registerTool(
  "cad_import_file",
  {
    description: "Import an existing STL, STEP, or OBJ file into the model registry.",
    inputSchema: {
      path:        z.string().min(1),
      output_name: ModelName.optional(),
    },
